...
def _uniform_source_axis(spec: BenchmarkCaseSpec) -> np.ndarray:
    components = _load_veqpy_components()
    route_spec = components["validate_route"](spec.mode, spec.coordinate, spec.input_kind)
    source_parameterization = getattr(route_spec, "source_parameterization", None)
    if source_parameterization is None:
        source_parameterization_for_route_key = components.get(
            "source_parameterization_for_route_key"
        )
        if source_parameterization_for_route_key is not None:
            source_parameterization = source_parameterization_for_route_key(
                (spec.mode, spec.coordinate, spec.input_kind)
            )
        else:
            source_parameterization = "identity"
    if source_parameterization == "sqrt_psin":
...
def load_high_order_validation_module():
    """Load scripts/06-high-order-reconstructions.py so Table 4 uses the same shape-error convention as Table 3."""
    module_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "06-high-order-reconstructions.py"
    )
