...
def veq_transport_geometry(
    ...
    *,
    ...
    r_gtt_over_j = np.asarray(geom.gttdivJR, dtype=np.float64) * np.asarray(geom.R, dtype=np.float64) ** 2
    metric_rho = 2.0 * np.pi * psin_r * np.asarray(eq.grid.quadrature(r_gtt_over_j, axis=1), dtype=np.float64)
...
def solve_heat_profile(geometry: TransportGeometry, source: np.ndarray) -> TransportResult:
    ...
    beta_proxy = float(2.0 * MU0 * np.trapezoid(geometry.vprime * temperature, x) / (volume * geometry.b0**2))
...
def solve_signature_equilibrium(fig08, benchmark, case_key: str, boundary, geqdsk, signature: dict[str, int]):
    grid = benchmark.Grid(
        Nr=32,
        Nt=32,
    ...
    profile_coeffs = {name: values for name, values in raw_profile_coeffs.items() if values is not None}
    ...
    return solver.build_equilibrium(), float(solver.result.elapsed) / 1000.0, int(np.size(solver.result.x))
...
def level_line_style(case_key: str, level_label: str) -> tuple[str | tuple[int, tuple[float, ...]], str]:
...
def plot_results(case_results: list[tuple[str, list[TransportResult]]], output_png: Path, output_pdf: Path) -> None:
    apply_plot_style()
...
def prepare_case_reference(
    ...
    if case_key == "solovev":
        reference_geometry = analytic_line_integrals(fig07, x_eval, label=REFERENCE_LABELS[case_key])
    else:
...
def solve_case_results(fig07, fig08, benchmark, case_key: str, x_eval: np.ndarray) -> list[TransportResult]:
    geqdsk, boundary, reference_geometry = prepare_case_reference(fig07, fig08, benchmark, case_key, x_eval)
    veq_geometries = []
    for signature in CASE_SIGNATURES[case_key]:
        eq, elapsed_ms, params = solve_signature_equilibrium(fig08, benchmark, case_key, boundary, geqdsk, signature)
...
def main() -> None:
    ...
    case_results = [(case_key, solve_case_results(fig07, fig08, benchmark, case_key, x_eval)) for case_key in CASE_KEYS]
